...
@Component
@Slf4j
//todo: 将MetricsAspect这个Bean的优先级设置为最高
...
public class MetricsAspect {
    ...
    // 默认值。这里为了减少代码量用了一个小技巧，即通过初始化一个具有1个元素的数组，然后通过获取这
    // 个数组的值来获取基本类型默认值
    ...
    // @Pointcut("@annotation(org.crayzer.err.coding.srpingpart1.aopmetrics.Metrics)")
    ...
    @Pointcut("within(@org.crayzer.err.coding.springpart1.aopmetrics.Metrics *)")
    public void withMetricsAnnotation() {
    }
    ...
    public Object metrics(ProceedingJoinPoint pjp) throws Throwable {
        ...
        // 切入的连接点是方法，注解定义在类上是无法直接从方法上获取到注解的。修复方式是，改为优先
        // 从方法获取，如果获取不到再从类获取,，如果还是获取不到再使用默认的注解
        ...
        // 因为需要默认对所有@RestController标记的Web控制器实现
        // @Metrics注解的功能，在这种情况下方法上必然是没有 @Metrics注解的，需要获取一个默认注解。
        // 虽然可以手动实例化一个@Metrics注解的实例出来，但为了节省代码行数，我们通过在一个内部类
        // 上定义@Metrics注解方式，然后通过反射获取注解的小技巧，来获得一个默认的@Metrics注解的实例
        if (metrics == null) {
            @Metrics
            final class c {
            }
